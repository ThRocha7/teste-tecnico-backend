...
export const registerLead = async (req: Request, res: Response) => {
  try {
    ...
    const resultSendEmail = await welcomeEmail(req.body.email, result.body.username);
    result.sent_email = resultSendEmail;
    return res.status(201).json({ message: result });
  } catch (error) {
    if (error.message === "duplicate key value violates unique constraint \"leads_email_key\""){
